          ...
          >
            ...
            <template #append>
              <v-switch
                ...
                density="compact"
                class="mr-4"
                @update:model-value="toggleNotification(notif.notification_id, $event as boolean)"
              />
              <v-btn icon="mdi-pencil" variant="text" size="small" class="mr-1" @click="openEditDialog(notif)" />
              <v-btn icon="mdi-delete" variant="text" size="small" color="error" @click="deleteNotification(notif.notification_id)" />
            </template>
          </v-list-item>
        </v-list>
      ...
      <v-dialog v-model="dialogOpen" max-width="560" persistent>
        <v-card>
          ...
          <v-card-actions class="pa-4">
            ...
            <v-btn color="primary" variant="flat" :disabled="!isFormValid" @click="saveNotification">
              保存
            </v-btn>
          </v-card-actions>
        </v-card>
...
function saveNotification() {
  if (editingNotification.value) {
    const idx = sitesStore.notifications.findIndex(
      (n) => n.notification_id === editingNotification.value!.notification_id,
    )
    if (idx !== -1) {
      sitesStore.notifications[idx] = {
        ...sitesStore.notifications[idx],
    ...
    sitesStore.notifications.push(newNotification)
    showSnackbar('通知先を追加しました')
  }
  closeDialog()
}

function toggleNotification(notificationId: string, enabled: boolean) {
  const idx = sitesStore.notifications.findIndex((n) => n.notification_id === notificationId)
  if (idx !== -1) {
    sitesStore.notifications[idx] = { ...sitesStore.notifications[idx], enabled }
  }
}
...
function deleteNotification(notificationId: string) {
  const idx = sitesStore.notifications.findIndex((n) => n.notification_id === notificationId)
  if (idx !== -1) {
    sitesStore.notifications.splice(idx, 1)
    showSnackbar('通知先を削除しました')
  }
}

function sendTestNotification(type: 'email' | 'slack') {
  const label = type === 'email' ? 'テスト通知メール' : 'テストSlack通知'
  showSnackbar(`${label}を送信しました（モック）`)
}
