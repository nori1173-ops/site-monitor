...
```bash
curl -X DELETE https://uexyis2uh1.execute-api.ap-northeast-1.amazonaws.com/Prod/users/me \
  -H "Authorization: Bearer $COGNITO_TOKEN"
```
...
export ADMIN_AUTH=$(echo -n 'admin:osasi034' | base64)
```

### GET /admin/users — ユーザー一覧取得

```bash
curl -s https://uexyis2uh1.execute-api.ap-northeast-1.amazonaws.com/Prod/admin/users \
  -H "Authorization: Bearer $COGNITO_TOKEN" \
...
```bash
curl -X POST https://uexyis2uh1.execute-api.ap-northeast-1.amazonaws.com/Prod/admin/users/user@osasi.co.jp/toggle-status \
  -H "Authorization: Bearer $COGNITO_TOKEN" \
...
```bash
curl -X POST https://uexyis2uh1.execute-api.ap-northeast-1.amazonaws.com/Prod/admin/users/user@osasi.co.jp/reset-password \
  -H "Authorization: Bearer $COGNITO_TOKEN" \
  -H "X-Admin-Auth: Basic $ADMIN_AUTH"
```
...
```bash
curl -X DELETE https://uexyis2uh1.execute-api.ap-northeast-1.amazonaws.com/Prod/admin/users/user@osasi.co.jp \
  -H "Authorization: Bearer $COGNITO_TOKEN" \
  -H "X-Admin-Auth: Basic $ADMIN_AUTH"
```
