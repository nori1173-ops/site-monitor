...
| ADMIN_CREDENTIALS | 管理者認証情報（user:password形式） |
...
- ユーザーの有効化/無効化（トグルスイッチ）
- パスワードリセット（ユーザーに確認コード付きメールが届く）
- ユーザー削除（登録サイトが0件の場合のみ）
...
1. **管理者画面でアカウントを無効化**
   - 管理者画面で対象ユーザーの「無効化」ボタンをクリック
   - 無効化されたユーザーはログインできなくなる

2. **登録サイトの引き継ぎ（必要な場合）**
   - 退職者が登録したサイトを別のユーザーに引き継ぐ場合は、管理者認証を使用してサイトを更新
   - 管理者オーバーライドにより、他ユーザーのサイトの PUT/DELETE が可能
   - API経由: `X-Admin-Auth` ヘッダーを付与してサイトを更新

3. **アカウント削除**
   - 退職者の登録サイトがすべて削除または引き継ぎ完了後、管理者画面からアカウントを削除
   - DynamoDB上の `created_by` / `updated_by` の記録はそのまま残る（監査証跡）
...
2. 登録メールアドレスを入力
3. メールで届く確認コードを入力
4. 新しいパスワードを設定（8文字以上、英大小文字+数字）

**管理者によるリセット:**
1. 管理者画面にアクセス
2. 対象ユーザーの「パスワードリセット」ボタンをクリック
3. ユーザーにパスワードリセットの確認コードがメールで届く
4. ユーザーが次回ログイン時に新パスワードを設定

### 5.4 ユーザー自身によるアカウント削除

ユーザーはナビゲーションメニューの「アカウント削除」から自分のアカウントを削除できる。
ただし、登録サイトが0件である必要がある。サイトが残っている場合はエラーメッセージが表示される。

---

## 6. トラブルシューティング

### 6.1 DLQ (Dead Letter Queue) の確認
