...
```
ユーザー
  ↓ https://site-monitor.example-cloud.com
...
ユーザー → サインアップ画面
  ├── メールアドレス + パスワード入力
  ├── Pre Sign-up Lambda トリガー → @example.com ドメインチェック
  │   └── ドメイン不一致 → 拒否
...
```
ユーザー → ログイン画面「パスワードをお忘れですか？」
  ├── メールアドレスを入力
  ├── aws-amplify Auth.resetPassword() → Cognito
  ├── 確認コードがメールに届く
  ├── 確認コード + 新パスワードを入力
  ├── Auth.confirmResetPassword() → パスワード更新
  └── ログイン可能になる
```

※ 管理者画面からもパスワードリセットが可能（AdminResetUserPassword API使用）
...
管理者APIおよび管理者オーバーライド機能では、Cognito JWT に加えてベーシック認証（`X-Admin-Auth` ヘッダー）を使用する。

| 項目 | 値 |
|------|-----|
| ヘッダー名 | `X-Admin-Auth` |
| 形式 | `Basic <base64(username:password)>` |
| 認証情報 | CloudFormationパラメータ `AdminCredentials` で設定 |
| 対象 | `/admin/*` エンドポイント、サイトの PUT/DELETE オーバーライド |

### 管理者オーバーライド

管理者認証が有効な場合、通常は `created_by` が一致しないと403になるサイトの PUT/DELETE 操作が可能になる。退職者が登録したサイトの管理などに使用する。

### ユーザー削除時のデータ保全

ユーザーを削除（`DELETE /users/me` または `DELETE /admin/users/{email}`）した場合:

- Cognito User Pool からアカウントが削除される
- DynamoDB上の `created_by` / `updated_by` に記録されたメールアドレスはそのまま残る（監査証跡として保持）
- 削除前にサイト登録が0件であることが必須条件（サイトが残っている場合は400エラー）
...
| レイヤー | 実装箇所 | 対象 | 許可IP |
|---------|---------|------|--------|
| フロントエンド | CloudFront Function (viewer-request) | 全HTTPリクエスト | 203.0.113.1 |
| API | API Gateway リソースポリシー | REST API 全エンドポイント | 203.0.113.1 |
...
| サイト更新 (PUT /sites/{id}) | 作成者のみ（created_by == 操作者）、または管理者認証 |
| サイト削除 (DELETE /sites/{id}) | 作成者のみ、または管理者認証 |
| 通知設定の操作 | 対象サイトの作成者のみ |
| チェック結果・履歴参照 | 全認証ユーザー |
| テストチェック・テスト通知 | 全認証ユーザー |
| 自ユーザー削除 (DELETE /users/me) | 自分自身のみ（サイト0件が条件） |
| ユーザー管理 (/admin/users/*) | 管理者認証必須 |
...
| 管理者認証情報 | CloudFormation パラメータ（AdminCredentials） | Lambda 環境変数 |
...
| パラメータ名 | タイプ | 説明 |
|-------------|--------|------|
| /site-monitor/slack-webhook-url | SecureString | Slack Incoming Webhook URL |
...
| サインアップ制限 | @example.com ドメインのみ | Cognito Pre Sign-up Lambda |
| MFA | OPTIONAL (TOTP) | Cognito User Pool |
| 管理者認証 | ベーシック認証（X-Admin-Auth） | API Lambda |
