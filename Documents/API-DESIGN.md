...
Web死活監視システムのREST API設計。全13エンドポイントを単一Lambda関数で処理し、API Gateway + Cognito Authorizerで認証する。
...
| 方式 | ヘッダー | 対象 |
|------|---------|------|
| Cognito JWT | `Authorization: Bearer <ID Token>` | 全エンドポイント |
...
| 403 | Not authorized to modify this site | 作成者以外による更新/削除 | 作成者のアカウントで操作 |
