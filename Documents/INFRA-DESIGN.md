...
|-----------|-----|-----------------|--------|------|
| StackName | String | SiteMonitor-Dev | SiteMonitor | スタック名 |
| ProjectName | String | site-monitor | site-monitor | コスト配分タグ |
| ExamplePowertoolsPython | String | arn:aws:lambda:...:79 | 同左 | 社内Lambdaレイヤー ARN |
| LogLevel | String | DEBUG | INFO | Lambdaログレベル |
| SubDomain | String | site-monitor-dev | site-monitor | CloudFrontサブドメイン |
| AdminCredentials | String | admin:SecurePassword123 | 同左 | 管理者認証情報（user:password形式） |
| AllowedIpAddresses | CommaDelimitedList | 203.0.113.1 | 同左 | IP制限 |
| HostedZoneId | String | ZXXXXXXXXXXXXX | 同左 | Route 53 ホストゾーンID |
...
| Timeout | 30秒 |
| Architecture | x86_64 |
| Layers | ExamplePowertoolsPython |
...
| Pre Sign-up トリガー | CognitoPreSignUpFunction（`@example.com`ドメイン制限） |
...
| EMAIL_DOMAIN | alive.example-cloud.com |
...
| ses:SendEmail | alive.example-cloud.com（us-west-2） |
| ssm:GetParameter | /site-monitor/* |
...
| レコード | タイプ | 値 |
|---------|--------|-----|
| `{SubDomain}.example-cloud.com` | A (Alias) | CloudFront Distribution |
...
| リソース | 説明 |
|---------|------|
| EmailIdentity | `alive.example-cloud.com` ドメイン検証 |
| MailFrom | `bounce.alive.example-cloud.com` |
| Route 53 レコード | DKIM (CNAME x3) + SPF (TXT) + MailFrom (MX, TXT) |

### 送信元

```
Example.NET<noreply@alive.example-cloud.com>
```
...
stack_name = "SiteMonitor-Dev"
region = "ap-northeast-1"
s3_bucket = "example-sam-ap-northeast-1"
s3_prefix = "SiteMonitor-Dev"
confirm_changeset = false
parameter_overrides = [
    "StackName=SiteMonitor-Dev",
    "LogLevel=DEBUG",
    "SubDomain=site-monitor-dev",
]

[production.deploy.parameters]
stack_name = "SiteMonitor"
s3_prefix = "SiteMonitor"
confirm_changeset = true
parameter_overrides = [
    "StackName=SiteMonitor",
    "LogLevel=INFO",
    "SubDomain=site-monitor",
]
```
