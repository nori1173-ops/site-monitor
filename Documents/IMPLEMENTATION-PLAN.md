...
| 1-5 | Cognito Pre Sign-upトリガー | `@example.com` ドメイン制限Lambda |
...
| 1-9 | SES EmailIdentity（alive.example-cloud.com） | us-west-2にEmailIdentity + DKIM + SPF + MailFrom、Route53レコード自動管理 |
...
- [ ] Pre Sign-upトリガーで `@example.com` のみ許可か
- [ ] MFAが「任意（オプション）」設定か
...
- [ ] Route 53で `site-monitor.example-cloud.com` レコードが定義されているか
- [ ] samconfig.tomlにdev（DEBUG）/production（INFO）のLogLevel設定があるか
- [ ] 全リソースに `Project` + `Name` タグが付与されているか
- [ ] ExamplePowertoolsPython レイヤーARNがパラメータ化されているか
- [ ] SES EmailIdentity が `alive.example-cloud.com` で作成されているか（us-west-2）
- [ ] DKIM・SPF・MailFrom（`bounce.alive.example-cloud.com`）が設定されているか
...
- [ ] ExamplePowertoolsPython の LambdaLogger を使用しているか
...
| 4-2 | メール通知（SES） | 送信元: `Example.NET<noreply@alive.example-cloud.com>`、件名カスタマイズ、テンプレート処理 |
...
- [ ] SES送信元が `Example.NET<noreply@alive.example-cloud.com>` 形式か（netmail-backend準拠）
...
| ブラウザ | Chromium（ヘッドレス） |
| テスト対象 | https://site-monitor-dev.example-cloud.com/ （検証環境） |
| 実行環境 | WSL (Ubuntu)、社内IPとしてアクセス可能 |
| 認証アカウント | testuser@example.com |
| 通知送信先 | testuser@example.com（メール実送信確認） |
...
| # | シナリオ | 操作 | 期待結果 |
|---|---------|------|---------|
| J4-1 | メール通知設定追加 | 通知設定画面 → メール選択 → testuser@example.com入力 | 通知設定が保存される |
| J4-2 | Slack通知設定追加 | Slack選択 → SSMパラメータ名入力 → メンション設定 | 通知設定が保存される |
| J4-3 | テストメール送信 | テスト通知ボタン（メール） | testuser@example.comにテスト通知メール着信 |
...
{
  testDir: './tests/e2e',
  baseURL: 'https://site-monitor-dev.example-cloud.com',
  use: {
...
/orchestrate custom "planner,tdd-guide,code-reviewer" "Phase 9: Playwright E2Eテスト（検証環境 https://site-monitor-dev.example-cloud.com/）"
# → 最終設計適合チェック
```
...
| SES EmailIdentity未検証 | メール送信不可 | Phase 1完了後に `alive.example-cloud.com` のDKIM検証ステータスを確認 |
...
- [ ] テスト通知メールが testuser@example.com に実際に届くこと
...
- [ ] 状態変化履歴が画面で確認できること
- [ ] SES送信元が `Example.NET<noreply@alive.example-cloud.com>` であること
